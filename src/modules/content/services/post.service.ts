...
export class PostService {
  constructor(
    ...
    // 这里是可选的,在使用useFactory的inject数组里的写法就会有所不同,先这样理解
  ...
  async update(data: UpdatePostDto) {
    ...
    if (isArray(data.categories)) {
      ...
      await this.repository
        ...
        .of(post)
        .addAndRemove(data.categories, post.category ?? []);
    }
  ...
  protected async buildListQuery(
    ...
    if (!isNil(search)) await this.buildSearchQuery(qb, search);
